%% Indian Prines with SGD, no VarPro (EVP_IndianPines_SGD.m)
...
numClasses      = size(COrig,2) - 1;  % remove background
...
T       = 4;    % final time
nt      = 16;    % number of time steps (will not prolongate with nt=1)
...
for level = 1:nLevels
    [th0,his,xOptAcc,xOptLoss]  = solve(opt,fctn,th0,fval);
    HIS{level} = his;
...
% use optimal validation accuracy as theta
thOpt = xOptAcc;
WOpt      = reshape(thOpt(nTheta(net)+1:end),size(Ct,1),[]);
thOpt     = thOpt(1:nTheta(net));

if doSave
    save(resFile,'net','thOpt','WOpt','HIS','thLast','xOptLoss');
end
