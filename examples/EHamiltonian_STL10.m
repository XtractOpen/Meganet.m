...
if nargin==0
    opt = sgd('learningRate',1e-1,'maxEpochs',4,'out',1,'miniBatch',125);
    feval(mfilename,125,125,2,2,0,'single',opt,[]);
    return
end
...
for k=1:numel(h)
    ...
    % PDE layer
    nL    = tvNormLayer([nImgc(1:2) nf(k)/2],'isWeight',1);
    ...
    regD = gpuVar(useGPU,precision,repmat([ones(2*nTheta(K),1); zeros(2*nTheta(nL),1)],nt(k),1));
